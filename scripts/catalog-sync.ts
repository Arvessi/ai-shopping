...
function resolveStores(): CollectorStore[] {
  ...
  if (raw === 'rotation') {
    const size = integer(arg('storeLimit'), 6, 1, 12);
    const day = Math.floor(Date.now() / 86_400_000);
    const start = (day * size) % collectorStores.length;
    return Array.from({ length: Math.min(size, collectorStores.length) }, (_, index) => collectorStores[(start + index) % collectorStores.length]);
  }
...
const firstCursor = integer(arg('cursor'), 0, 0, 10_000_000);
const slices = integer(arg('slices'), 1, 1, 6);
const concurrency = integer(arg('concurrency'), 3, 1, 3);
const startedAt = Date.now();
const sliceResults: Array<{ cursor: number; results: any[] }> = [];

for (let slice = 0; slice < slices; slice += 1) {
  const cursor = firstCursor + slice * limit;
  ...
  console.error(`\nCENIQ sync slice ${slice + 1}/${slices}: ${stores.map((store) => store.slug).join(', ')} | limit=${limit} cursor=${cursor} concurrency=${concurrency}`);
  ...
  sliceResults.push({ cursor, results });
}

const allResults = sliceResults.flatMap((slice) => slice.results);
const summary = {
  slices,
  storesPerSlice: stores.length,
  storeRuns: allResults.length,
  successfulStoreRuns: allResults.filter((result) => result.accepted > 0).length,
  totalCollected: allResults.reduce((sum, result) => sum + result.collected, 0),
  totalAccepted: allResults.reduce((sum, result) => sum + result.accepted, 0),
  totalRejected: allResults.reduce((sum, result) => sum + result.rejected, 0),
  durationMs: Date.now() - startedAt,
  tavilyCalls: 0,
  dataForSeoCalls: 0,
  firstCursor,
  nextCursor: firstCursor + slices * limit,
};

console.log(JSON.stringify({ ok: true, summary, slices: sliceResults }, null, 2));
