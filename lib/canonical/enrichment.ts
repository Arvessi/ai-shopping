...
import { createShoppingTask, getShoppingTask, mapShoppingCandidates } from './dataforseo-client';

const DEADLINE_MS = Math.min(180_000, Math.max(30_000, Number(process.env.ENRICHMENT_DEADLINE_MS || 90_000)));
const MAX_ATTEMPTS = Math.min(20, Math.max(3, Number(process.env.ENRICHMENT_MAX_POLLS || 9)));
...
export async function queueEnrichment(query: string, familyId?: string) {
  ...
  const existing = await prisma.enrichmentJob.findFirst({
    where: { normalizedQuery, status: { in: ['queued', 'running'] }, deadlineAt: { gt: new Date() } },
    orderBy: { createdAt: 'desc' },
  });
  if (existing) return existing;
  return prisma.enrichmentJob.create({
    data: { normalizedQuery, familyId, deadlineAt: new Date(Date.now() + DEADLINE_MS) },
  });
}
...
export async function startEnrichment(query: string, familyId?: string) {
  ...
  try {
    ...
    job = await prisma.enrichmentJob.update({
      where: { id: job.id },
      data: { status: 'running', startedAt: new Date(), providerStage: 'products', providerTaskId, providerTaskIds: [providerTaskId] },
    });
  } catch (error) {
    job = await prisma.enrichmentJob.update({
      where: { id: job.id },
      data: { status: 'failed', finishedAt: new Date(), lastError: error instanceof Error ? error.message : 'Provider task creation failed' },
    });
  }
  return job;
}
...
async function terminal(jobId: string, status: 'failed' | 'timed_out', error: string) {
  return prisma.enrichmentJob.update({ where: { id: jobId }, data: { status, finishedAt: new Date(), lastError: error } });
}
...
export async function pollEnrichment(jobId: string) {
  ...
  if (job.status === 'succeeded') return { status: job.status, results: await searchCanonicalCatalog(job.normalizedQuery) };
  if (job.status === 'failed' || job.status === 'timed_out') return { status: job.status, error: job.lastError || undefined };
  if (!enrichmentLimitState({ deadlineAt: job.deadlineAt, attempts: job.attempts, maxAttempts: MAX_ATTEMPTS }).allowed) {
  ...
  if (job.status === 'queued' || !job.providerTaskId) return { status: 'queued' as const, retryAfterMs: 500 };

  const attempts = job.attempts + 1;
  await prisma.enrichmentJob.update({ where: { id: job.id }, data: { attempts } });
  try {
    const response = await getShoppingTask(job.providerTaskId);
    if (response.state.state === 'pending') {
      return { status: 'running' as const, retryAfterMs: Math.min(8_000, 750 * 2 ** Math.min(4, attempts - 1)) };
    }
