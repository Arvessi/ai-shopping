...
export default function SearchExperience() {
  ...
  async function runDataSearch(searchQuery: string, searchMode: 'search' | 'assistant') {
    setStatus('Meklējam un salīdzinām piedāvājumus…');
    setNotice('');
    ...
    try {
      ...
      setNotice(nextResults.length === 0 ? data.message || 'Nekas netika atrasts. Pamēģini precīzāku produkta nosaukumu.' : '');
      setStatus('');
    } catch (err) {
      if (err instanceof DOMException && err.name === 'AbortError') {
        throw new Error('Meklēšana šobrīd aizņem pārāk ilgi. Pamēģini vēlreiz pēc brīža.');
      }
      throw err;
    } finally {
    ...
    <>
      <section className="hero" id="meklet">
        ...
        <form className="bigsearch" onSubmit={(e: FormEvent<HTMLFormElement>) => submit(e)}>
          ...
          <input
            ...
            placeholder={mode === 'search' ? 'Piem., iPhone 17 Pro 256GB' : 'Piem., gaming monitors līdz 300 €, vismaz 144Hz un 27 collas'}
          />
      ...
      {plan && (
        <section className="container aianswer">
          ...
          <h2>{plan.summary}</h2>
          <div className="chips">{plan.constraints.map((constraint: string) => <span key={constraint}>{constraint}</span>)}</div>
        </section>
      )}
      ...
      {results.length > 0 && (
        <section className="container results">
          <div className="sectiontitle">
            <div><span>ATRASTIE PRODUKTI</span><h2>Labākās izvēles</h2></div>
            <p>{results.length} sagrupēti produkti</p>
          </div>
          <div className="productgrid">{results.map((product: ProductResult) => <ProductCard product={product} key={product.id} />)}</div>
        </section>
      )}
        ...
        <div className="popularRow">
          {popular.map((item: string, index: number) => (
            <button key={item} onClick={() => submit(undefined, item)}>
              <b>{String(index + 1).padStart(2, '0')}</b><span>{item}</span><i>↗</i>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="container section">
        ...
        <div className="categorygrid">
          {categories.map(([icon, label, q]) => (
            <button key={label} onClick={() => { setMode('search'); submit(undefined, q); }}>
      ...
      <section className="container how" id="ka-darbojas">
        ...
        <div className="howgrid">
          <article><b>01</b><h3>Meklē produktu</h3><p>Ieraksti modeli, ko jau zini. Ceniq grupē viena produkta piedāvājumus no dažādiem veikaliem.</p></article>
          <article><b>02</b><h3>Vai apraksti vajadzību</h3><p>Ceniq AI pārvērš tavas prasības konkrētos parametros un atrod atbilstošākos produktus.</p></article>
          <article><b>03</b><h3>Izvēlies gudrāk</h3><p>Cena, piegāde un tirgotāja signāli tiek apvienoti Ceniq vērtējumā, lai lētākais nebūtu vienīgais kritērijs.</p></article>
        </div>
      </section>
    </>
  );
